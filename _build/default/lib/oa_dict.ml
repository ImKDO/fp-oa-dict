...
module OA_Dict = struct
  ...
  type 'v t = {
    size: int;
    buckets: 'v slot array;
  }
  
  let make_empty () = {
    size = 0;
    buckets = Array.make 16 None;
  }
  
  let capacity d = Array.length d.buckets
  
  let load_factor d = 
    float_of_int d.size /. float_of_int (capacity d)
  ...
  let concat d1 d2 =
    fold_left d1 (fun acc k v -> insert k v acc) d2
  
  let size d = d.size
  
  let to_list d =
    fold_left [] (fun acc k v -> (k, v) :: acc) d |> List.rev
